...
export class LoadListComponent {
  ...
  brands = computed(() => {
    const data = this._brandService.filterBrands(this.searchText());
    if (this.dataSource) {
  ...
  selectAllTenants() {
    ...
    this.toggleActions();
    this.brands().forEach((row) => this.selection.select(row));
  }
  ...
  remove(brand?: BrandModel) {
    const multi: boolean = brand === null || brand === undefined;
    this._confirmationDialogs
      .confirmDelete("Brand", multi)
      .afterClosed()
      .subscribe((result) => {
        if (result === CONSTANTS.CONFIRMED) {
          this._brandService.removeSelectedBrands(
            brand ? [brand] : this.selection.selected
          );
          this.clearSelection();
        }
      });
